...
```
#### Порт закрыт
Означает, что ни одно из приложений сервера не прослушивает этот порт.
```console
...
#### Connection timeout
Вероятное причины сообщения Connection timed out: 
+ узел не в сети (физически или логически отключен от сети) или 
+ узел недостижим:
...
```
*Примечание*
1. Команда ```time``` добавляет информацию о времени выполнения команды, но не изменяет её поведение. Сообщение ```Connection timed out``` появилось бы в любом случае, если соединение не удалось установить из-за тайм-аута
