...
+ ```id``` - Выводит сведения о пользователе:<a id="06id"></a>
  ...
  + Отображения реальных идентификаторов пользователя и группы, а не эффективных: `-r`. Например, `id -r -u username` покажет реальный идентификатор пользователя.
    ...
    + Эффективный идентификатор пользователя (Effective User ID, EUID): Эффективный UID может отличаться от реального UID, если с помощью `setuid` были изменены права доступа для выполнения определенных операций. Например:
      + с помощью `sudo chown root:root example` и `sudo chmod u+s example` на исполняемый файл (`example`) был установлен бит `setuid` чтобы он выполнялся с правами владельца файла (`root`), а не с правами пользователя, который его запускает. Теперь если пользователь запустит команду `id` в процессе, запущенном файлом с `setuid` (у нас это `example`), то получит эффективный идентификатор пользователя, соответствующий `root` (`ID root`).
...
+ `cp`	-	используется для копирования файлов и директорий
  ...
  + Копирование каталога рекурсивно *вариант 2*: `cp -r source_directory/* /path/to/directory/`. В результате в каталоге `/path/to/directory/` появится только содержимое каталога `source_directory` и его подкаталогов
...
+ `mv` - используется для перемещения или переименования файлов и директорий
...
`sudo groupmod -n newgroupname oldgroupname` - изменение имени группы с oldgroupname на newgroupname  (автоматически вносит изменения в `/etc/group` и `/etc/gshadow`).
##### Изменение GID группы:
`sudo groupmod -g 2000 mygroup` - изменение GID группы mygroup на 2000 (автоматически вносит изменения в `/etc/group` и `/etc/gshadow`, но файл `/etc/passwd` вам может понадобиться обновить соответствующие записи в этом файле вручную, если вы хотите, чтобы основная группа пользователя совпадала с новым `GID` группы).
...
+ `id` - [Получить расширенную информацию о пользователе и его членстве в группах](#06id)
...
+ `-m` или `--create-home`: Указывает системе создать домашний каталог для нового пользователя
  + `useradd` с `-m`, но без `-d`: Создает домашний каталог по умолчанию в `/home` с именем пользователя. Например, `sudo useradd -m newuser` создаст каталог `/home/newuser`).
  + `useradd` без `-m` и `-d`: домашний каталог для нового пользователя не будет создан (в результате выполнения команды `sudo useradd newuser` будет создан пользователь `newuser`, но ему автоматически не будет выделен каталог в `/home` или любом другом месте).
+ `-d` или `--home` : Указывает путь к домашнему каталогу нового пользователя. Это говорит системе, где именно нужно создать домашний каталог, если опция `-m` тоже указана, или какой каталог использовать, если он уже существует
  + `useradd` с `-d`, но без `-m`: `sudo useradd -d /custom/home/dir newuser` - Указывает в качестве домашнего каталога `/custom/home/dir`, но не создает его. Если каталог не существует, он не будет создан автоматически
+ `-s` или `--shell` : Указывает оболочку для нового пользователя `sudo useradd -s /bin/bash newuser`.
+ `-g` или `--gid`: Указывает основную группу для нового пользователя (можно указать как GID, так и имя группы) `sudo useradd -g users newuser`.
...
+ `-d` или `--home HOME_DIR`: Изменяет домашний каталог пользователя `sudo usermod -d /new/home/john -m john` (в основном применяется с `-m`). Без `-m` новый домашний каталог будет назначен в системе, но содержимое старого каталога не будет автоматически перемещено
+ `-m` или `--move-home`: Перемещает содержимое домашнего каталога в новое место. Использование `-m` без `-d` не имеет смысла, так как `-m` перемещает содержимое домашнего каталога в новое местоположение, и это новое местоположение должно быть указано с помощью `-d`
+ `-g` или `--gid GROUP`: Изменяет основную группу пользователя `sudo usermod -g developers john`. В качестве аргумента можно указыватьт только имя основной группы пользователя.
+ `-G` или `--groups GROUPS`: Задает дополнительные группы для пользователя `sudo usermod -aG sudo,adm john` (без `-a` пользователь `john` будет удалён из всех остальных групп, к которым он принадлежал ранее). В качестве аргумента можно указывать только имена групп пользователя.
...
Команды `useradd` и `usermod` не задают пароль пользователя. 

После создания пользователя пароль должен быть установлен командой `sudo passwd имя_пользователя`. Это вызовет запрос на ввод и подтверждение нового пароля.
...
1. `su - sutest` - запустит оболочку от имени пользователя `sutest`. Опция `-` (или `-l`) указывает `su` загрузить окружение целевого (здесь `sutest`) пользователя, что означает:
   1. сменить домашний каталог на каталог целевого пользователя (в примере — `/home/sutest`)
   2. загрузить переменные окружения (`HOME`, `USER`, `LOGNAME`, `SHELL`, `PATH` и другие) целевого пользователя (в примере — `sutest`)
   3. обновить переменную `PATH`, чтобы включать пути, специфичные для целевого пользователя (в примере — `sutest`)
   4. загрузить и выполнить файлы конфигурации оболочки целевого пользователя (в примере — `sutest`), такие как `.profile`, `.bash_profile`, `.bashrc` и другие, если они существуют
2. `su - -c "id -u; whoami"` - выполнение команды (или нескольких, разделенных точкой с запятой команд) от имени целевого пользователя (в примере - `root`), без запуска полнофункционального сеанса оболочки `root`
  
**Примечания**
+ команда `su` - переключит на `root`
+ пользователь `root` может переключаться на любого другого без ввода пароля целевого пользователя.
...
+ Некоторые привилегированные пользователи (например, члены групп `wheel` или `sudo`) могут выполнять любые команды от имени `root`. В файле `/etc/sudoers` это может отображаться как:
  + `root ALL=(ALL:ALL) ALL`
    + `root`: Указывает, что правило применяется к пользователю `root`.
    + первый `ALL`: Указывает, что правило действует на всех хостах (компьютерах).
    + `(ALL:ALL)`: 
      + Первая часть `ALL`: Указывает, что `root` может выполнять команды от имени любого пользователя (заменять пользователем).
      + Вторая часть `ALL`: Указывает, что `root` может выполнять команды от имени любой группы (заменять группой).
    + Последний `ALL`: Указывает, что `root` может выполнять любую команду.
  + `john myhost=(alice) /usr/bin/apt-get, /usr/bin/systemctl`
    + `john`: Указывает, что правило применяется к пользователю `john`.
    + `myhost`: Конкретный хост, на котором действует это правило.
    + `(alice)`: Пользователь, от имени которого `john` может выполнять команды.
    + `/usr/bin/apt-get, /usr/bin/systemctl`: Список команд, которые `john` может выполнять от имени `alice`.
  + `%sudo ALL=(ALL) ALL`
    + `%sudo`: Указывает, что правило применяется ко всем пользователям, которые являются членами группы `sudo`. В sudoers записи, префикс `%` используется для обозначения групп.
    + `ALL`: Указывает, что правило действует на всех хостах (компьютерах).
    + `(ALL)`: Первая часть `ALL`: Указывает, что члены группы `sudo` могут выполнять команды от имени любого пользователя.
    + Последний `ALL`: Указывает, что члены группы `sudo` могут выполнять любые команды.
...
+ Выше результат команды `ls -l testfile`, где:
	+ `-` для типичных файлов / `d` для каталогов / `l` для символических ссылок
	+ 9 следующих символов (`rw-r--r--`) - разрешения доступа к файлам UNIX
...
|:-------------|:---------------:|--------------:|
| `r` - read (чтение)       |Файл может быть прочитан, например, с помощью `cat`, `less`, `head`, других инструментов.        | Список содержимого каталога может быть прочитан: `ls имя_каталога` для отображения содержимого каталога, но при этом, если у вас  не установлен бит `x` на каталог, не будет возможности просматривать подробную информацию о файлах и подкаталогах (например, `ls -l`), заходить в подкаталоги, читать файлы (`cat example_dir/example_file`) или запускать на выполнение файлы, которые находятся внутри этого каталога, т.к. при обращении к файлу система сначала проверяет право `x` у всех директорий, стоящих в пути этого файла, и только затем права на сам файл. Если хотя бы у одной директории право `x` отсутствует, доступ к этой директории и всему ее содержимому для Вас запрещается.        |
...
+ изменить только группу объекта - обычный пользователь может изменить группу объекта, если он является владельцем файла и одновременно членом новой группы. В противном случае, для изменения группы ему понадобятся привилегии суперпользователя (`root`)
...
#### `chgrp`
`chgrp [OPTIONS] GROUP FILE(s)`: для изменения группы объекта (вы должны быть владельцем файла и являться членом новой группы или иметь права `root`)
...
`chmod` позволяет как установить новые точные разрешения сразу на определенный объект - для этого лучше использовать восьмеричную форму, так и добавить или отозвать определенные права доступа к объекту - это удобнее делать в символьном виде
 + символьный вид: `chmod [OPTIONS] [ugoa…][-+=]perms…[,…] FILE(s)`
   + `[OPTIONS]` - аналогичны опциям `chown`, кроме:
     + `-X`: `sudo chmod -R -X /path/to/directory` - только для директорий и исполняемых файлов. Устанавливает бит выполнения, если файл уже имеет бит выполнения для кого-либо из пользователей. Это полезно при рекурсивном изменении прав доступа, чтобы не делать исполняемыми все файлы, а только те, которые уже были исполняемыми. **ВАЖНО!!!** Параметр `-X` добавляет право выполнения всем категориям пользователей (владельцу, группе и другим), если хотя бы одна из этих категорий уже имеет это право. Он не может быть использован для добавления права выполнения только для конкретной группы пользователей. Но если вам нужно добавить право выполнения только для конкретной группы, например, `user` и только тем файлам, которые уже исполняемые (т.е. для одного из классов: `group` или `other` установлен бит `x`), то использование команды [`find`](#findlnx) с `chmod` будет наиболее подходящим решением: `find directory -type f -perm -u=x -exec chmod g+x {} \;`
   + Первый набор флагов (`[ugoa...]`), флаги пользователей, определяет, для каких классов пользователей изменяются права доступа к файлу.
     ...
     + `a` - Все пользователи, идентично `ugo`, например:
       ...
       + `chmod -R go-rwx ~/chmod.me.dir` или `chmod -R go= ~/chmod.me.dir`- отнимаем рекурсивно для всей директории все права у группы и прочих
   ...
   + Разрешения `(perms...)` могут быть явно заданы с помощью нуля или одной или нескольких из следующих букв:
     + `r`: Чтение.
     + `w`: Запись.
     + `x`: Выполнение (или переход в каталог для директорий).
     + `s`: Установочный бит (`setuid` или `setgid`).
     + `t`: Бит `sticky`.
     + `X`: Бит выполнения для каталогов и файлов, которые уже имеют бит выполнения для кого-либо из пользователей. `chmod -R u+X directory` - рекурсивно добавит право на выполнение для пользователя (владельца), только если файл уже имеет право на выполнение для кого-то из пользователей, или если это директория (более точная альтернатива опции `-X`)
...
+ восьмеричная форма: `chmod [OPTIONS] NUMBER FILE(s)`, например:
...
+ для пересоздания символической ссылки требуется право на выполнение (`x`), чтобы "войти" в директорию и создать новую ссылку. Также нужно право на запись (`w`), чтобы изменять содержимое директории.
...
`ls -ld /tmp` вернет `drwxrwxrwt 10 root root 4096 Nov 19 16:00 /tmp`
...
Службы — фоновый процесс, который выполняет определённые задачи и обычно управляется системой `systemd` (`init` на прежних дистрибутивах Linux). Отвязаны от терминала (`tty` - не определено, ps покажет `?`)

### Systemd

`Systemd` (принят как замена системы инициализации `SysV init`) - комплект ПО, ответственный за инициализацию системы и управление объектами, называемыми юнитами (`services`, `targets`, `sockets`, `timers` и др.)

### Служба в контексте `Systemd`

1. Определяется через файл `.service`, который `systemd` использует для управления процессом или приложением, содержащим следующие разделы:
   1. `[Unit]` содержит общую информацию о юните и его зависимостях
   2. `[Service]` используется для определения параметров и команд, необходимых для запуска, остановки и перезапуска сервисов.
   3. `[Install]` описывает, как и когда юнит должен быть установлен активирован в системе.
2. Файлы `.service`, используемые `Systemd`, обычно находятся в следующих каталогах:
   1. `/lib/systemd/system/` - Этот каталог содержит стандартные юниты, поставляемые с дистрибутивом Linux. Сюда входят службы, которые устанавливаются вместе с операционной системой и различными пакетами.
   2. `/etc/systemd/system/` - Этот каталог предназначен для пользовательских и административных конфигураций. Если вы создаете или изменяете файлы юнитов, рекомендуем сохранять их именно здесь. Эти конфигурации имеют приоритет над теми, что находятся в `/lib/systemd/system/`
3. Служба `systemd` - это программа или процесс, который запускается и работает на основе настроек в файле `.service`. Когда вы запускаете службу через `systemctl start <имя_службы>`, `systemd`:
   1. Считывает настройки из соответствующего `.service-файла`.
   2. Запускает указанный процесс.
   3. Следит за состоянием этого процесса и управляет им в соответствии с заданными параметрами.
4. Пример структуры файла `.service` (Unit-файла сервиса)
   1. `[Unit]`
      1. `Description` - Краткое описание сервиса
      2. `Documentation` - URL-документации
      3. `Requires` - Содержит перечень юнитов, которые должны быть активны для запуска данного юнита (При запуске нашего юнита `systemd` действительно будет пытаться запустить все указанные юниты). Если один из них не удается запустить или он завершился неудачно, текущий юнит также завершится
      4. `Conflicts` - Обратное к `Requires`. Определяет юниты, которые не могут быть активными одновременно с текущим юнитом. Если один из них активируется, другой будет остановлен
      5. `Wants` - Содержит перечень юнитов, которые будут пытаться запускаться вместе с текущим юнитом, но если один из них не удается запустить, это не приведет к ошибке текущего юнита
      6. `After` - Определяет порядок запуска. Текущий юнит начнет запускаться после юнитов из поля `After`. Однако это не означает, что указанные юниты должны быть уже запущены, а только то, что текущий юнит не будет запускаться раньше них
      7. `Before` - Обратное к After
      8. `ConditionPathExists` - Содержит путь. Если указанный путь не существует, юнит не будет запуска
   2. `[Service]`
      1. `Type` - Содержит один из типов службы
         1. `simple` (по умолчанию): Процесс, указанный в `ExecStart`, запускается и остаётся активным;
         2. `forking`: Процесс запускает другую программу и завершается;
         3. `oneshot`: Для служб, которые выполняются и завершаются сразу;
         4. `dbus`: Процесс должен зарегистрироваться с `D-Bus`;
         5. `notify`: Процесс уведомляет `systemd` о своём состоянии;
         6. `idle`: Выполняется, когда загрузка системы завершена
      2. `ExecStart` - основная команда для запуска сервиса. Это обязательное поле
      3. `ExecStop` - команда для остановки сервиса
      4. `ExecReload` - команда для перезагрузки сервиса
      5. `Restart` - указывает, как `service manager` должен поступать в случае аварийного завершения службы:
         1. `no` - Служба не будет перезапущена, независимо от причины завершения. Это поведение по умолчанию
         2. `on-success` - Служба будет перезапущена только в случае её успешного завершения, то есть если она завершилась с кодом 0 или сигнала TERM
         3. `on-failure` - Служба будет перезапущена в случае её неуспешного завершения, то есть если она завершилась с любым кодом, отличным от 0, или если она была завершена сигналом, отличным от TERM
         4. `on-abnormal` - Служба будет перезапущена, если она была завершена из-за сигнала, который не был вызван системой управления службами. Например, если служба была завершена из-за сигнала SEGV или ABRT
         5. `on-abort` - Служба будет перезапущена, если она была завершена из-за получения сигнала ABRT
         6. `always` - Служба будет перезапущена в любом случае, независимо от причины её завершения
      6. `ExecStartPre` - команда, выполняемая перед запуском основного процесса. Полезно для выполнения предварительных задач, таких как проверка условий или подготовка среды
      7. `ExecStartPost` - команда, выполняемая после успешного запуска основного процесса
      8. `RestartSec` - определяет задержку (в секундах) перед перезапуском службы после ее неуспешного завершения или остановки. Это поле используется совместно с параметром `Restart` и помогает предотвратить слишком частые перезапуски службы в случае, если она часто выходит из строя
      9. `User` - пользователь, от имени которого запускается сервис
      10. `Group` - группа, от имени которой запускается сервис
      11. `WorkingDirectory` - рабочий каталог для процесса
   3. `[Install]`
      1. `WantedBy` - содержит `target`, указывая `systemd` запускать ваш сервис вместе с этой целью (`target`). Но если ваш сервис не сможет запуститься это не будет критически важным для цели
      2. `RequiredBy` - содержит `target`, указывая `systemd` запускать ваш сервис вместе с этой (`target`). Но если ваш сервис не сможет запуститься это будет критически важным для цели и цель не будет считаться полностью активированной без вашего юнита
      3. `Also` - содержит дополнительные юниты, которые должны быть установлены вместе с текущим юнитом (При выполнении команды `systemctl enable example.service`, `systemd` также создаст символические ссылки для всех юнитов, указанных в поле `Also`. Таким образом, все связанные юниты будут включены для запуска при старте системы.)
      4. `Alias` - содержит другие имена для этого юнита

#### Пример на практике

1. Возьмём службу `SSH`: файл `/lib/systemd/system/ssh.service`:
...
2. Запуск команды: `sudo systemctl start ssh`
   1. `systemd` считывает файл `ssh.service`.
   2. Запускается процесс `/usr/sbin/sshd -D`.
3. Теперь `sshd` становится работающей "службой".

### Вывод списка активных служб (units) в системе
...
+ `LOAD`: показывает статус загрузки конфигурационного файла сервиса. Возможные значения:
  ...
  + `masked`: Сервис замаскирован. Это означает, что соответствующий сервис был отключен и заблокирован от запуска. Маскировка сервиса предотвращает его активацию, даже если кто-то попытается его запустить вручную. Это может быть полезно для предотвращения запуска сервисов, которые могут повредить систему или создать угрозу безопасности
+ `ACTIVE`: показывает общий статус активности сервиса. Возможные значения:
  + `active`: Сервис запущен и работает нормально.
  + `reloading`: Сервис в процессе перезагрузки конфигурации (без полного остановки и перезапуска). Это состояние обычно возникает, когда используется команда `ExecReload` в конфигурации сервиса
  + `inactive`: Сервис остановлен и не запущен.
...
**ВНИМАНИЕ** `--state` в качестве значений для фильтрации вывод может содержать значения из полей `LOAD`, `ACTIVE` и `SUB`. К сожалению, фильтрация возможна только по одному критерию одновременно, чтобы отфильтровать юниты по двум критериям, таким как `loaded` и `inactive`, вы можете использовать команду `systemctl list-units` в сочетании с командой `grep`: 

`systemctl list-units --type=service --all --state=loaded | grep 'inactive'`
...
+ `masked`: Служба замаскирована это значит, что она заблокирована для запуска и не может быть запущена ни вручную, ни автоматически, пока не будет размаскирована. Замаскированная служба фактически отключена и недоступна для выполнения.
+ `indirect`: Указывает на то, что файл единицы является косвенным, т.е не активируется напрямую, а используется как часть другого unit-файла
+ `generated`: служба была создана динамически во время запуска системы. Это обычно происходит автоматически, когда система создает временные или динамические единицы.
+ `alias`: Служба является псевдонимом другой службы. Это позволяет использовать более короткое или удобное имя для ссылки на существующую единицу. Для создания псевдонима применяется команда `systemctl alias myservice.service`

Каждой службе может соответствовать только одно состояние в `systemctl list-unit-files --type=service` на основе приоритетов и логики системного менеджера `systemd`. Если сервис может соответствовать нескольким состояниям, например, если он одновременно `enabled` и `masked`, то вывод будет выбран в соответствии с приоритетами `systemd`:

1. `masked` 2. `enabled` 3. `disabled` 4. `static` 5. `alias` 6. `indirect` 7. `generated`

`systemctl list-unit-files --type=service --state=enabled` - выведет список всех файлов юнитов типа service, которые настроены на автоматический запуск (то есть, имеют состояние `enabled`).

### Управление службами 

+ `systemctl status название_службы` - Проверка текущего состояния службы (по названию)
+ `systemctl status PID_процесса` - Проверка текущего состояния службы (по процессу)
+ `systemctl start название_службы` - Запуск службы (необходимы права root)
+ `systemctl stop название_службы` - Остановка службы (необходимы права root)
+ `systemctl restart название_службы` - Перезапуск службы (необходимы права root)
+ `systemctl enable|disable [--now] название_службы` - Включение и отключение автоматического запуска службы на этапе загрузки ОС. Опция `--now` используется, чтобы добавить службу в автозагрузку и одновременно запустить её или исключить из автозагрузки и одновременно остановить (в зависимости от состояния службы). (необходимы права root)
+ `systemctl mask|unmask [--now] название_службы` - применение `mask` маскирует службу (создает символическую ссылку от название_службы к `/dev/null`, что предотвращает запуск службы), применение `unmask` размаскирует службу (удаляет символическую ссылку, позволяя службе снова запускаться).
+ `systemctl is-enabled название_службы` - Узнать настройки автоматического запуска конкретной службы
