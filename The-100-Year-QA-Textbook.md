...
+ `Stderr` перенаправляется в stdout (который уже к этому времени на предыдущем шаге был перенаправлен в output_file)
...
+ в ```cat /etc/group | egrep user "cat"``` просто печатает входной файл в `stdout`. Затем, с помощью пайпа через `|` между `cat` и `egrep`, это содержимое передается `egrep` в виде потока на `stdin`. `egrep` читает свой `stdin` и ищет заданный шаблон.
...
+ `2> >(next_command)`: перенаправляет стандартный поток ошибок (`stderr`) через временный именованный канал в стандартный ввод (`stdin`) команды next_command .
