...
+ `-g`, `--pgroup <pgrp>`: Ищет процессы в указанных **процессных группах**. <a id="pgroup"></a>
  ...
  + Все процессы в группе могут управляться единым сигналом (например, с помощью команды `kill` или `pgrep` или `pkill` с флагом `-g`
...
+ по умолчанию ищет процессы по командной строке при использовании флага `-f` (заставляет команду искать во всей командной строке, а не только в имени процесса)
+ синтаксис:
...
+ `-n, --ns` - Искать процессы в том же пространстве имён `PID` что и у `PID` (без флага `-n, --ns` по умолчанию поиск выполняется по всем пространствам имён):
