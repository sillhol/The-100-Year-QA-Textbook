...
3. POST-запросы
   ...
   2. ```curl -X POST -H "Content-Type: application/json" -d '{"username":"user", "password":"pass"}' http://example.com/login``` - отправка POST-запроса с JSON-данными
4. Аутентификация
...
+ Характеристики:
  ...
  + Включает механизмы аутентификации сервера и иногда клиента (используется для обеспечения дополнительного уровня безопасности и доверия при доступе к критически важным ресурсам. Она настраивается на сервере, который запрашивает и проверяет клиентские сертификаты, и требует наличия соответствующих сертификатов на стороне клиента).
...
+ Характеристики:
  ...
  + Сервер может инициировать передачу данных (Server Push), что позволяет отправлять данные клиенту до того, как он их запросит (добавлена вместо сокета новая сущность "логические потоки", через которую сервер инициирует Server Push).
...
+ Характеристики:
  + Основан на протоколе QUIC - современном транспортном протоколе, разработанном для замены или дополнения традиционного TCP. QUIC работает поверх UDP и был разработан Google для обеспечения более быстрой и надежной передачи данных в интернете. Он обладает рядом улучшений по сравнению с TCP, особенно в контексте использования с протоколом HTTP/3:
...
+ HTTP/3: Еще более быстрая и эффективная передача данных благодаря использованию QUIC и работы поверх UDP, лучше подходит для современных сетей с высокой задержкой (не для условий, где недопустима потеря данных из-за потери пакетов).
+ HTTPS: Обеспечивает безопасность передачи данных, защищает от атак типа "человек посередине" (MITM) и обеспечивает конфиденциальность и целостность данных (но в большинстве современных браузеров и серверов при установке HTTPS-соединения автоматически включается HTTP/2).
### Stateless protocol
...
+ ```Strict-Transport-Security```: Указывает политику безопасности для HTTPS. Например, ```Strict-Transport-Security: max-age=31536000```. Когда браузер получает заголовок ```Strict-Transport-Security``` от сервера, он запоминает домен и обязуется в течение указанного периода времени (максимального возраста) использовать только HTTPS для всех последующих запросов к этому домену. Если злоумышленник попытается заставить браузер использовать HTTP, а не HTTPS, ```Strict-Transport-Security``` предотвратит это, так как браузер будет знать, что только HTTPS соединения разрешены
...
HTTP-запрос/ответ может содержать полезную нагрузку, которая помещается в часть Request Body или Response Body. Тело ответа может содержать исходный код веб-страницы или только что созданные данные учетной записи пользователя, например, в формате JSON, различные типы данных (медиатипы в соответствии со стандартом): аудио, изображение, текст и так далее. При выполнении запроса (PUT, POST) тип данных задается с помощью заголовка ```Content-type```:
+ ```Content-Type: text/html; charset=utf-8```
## JSON
JSON (JavaScript Object Notation) - легковесный и фундаментальный формат для создания современных веб-приложений и API: используется как для передачи данных между сервером и веб-приложением, так и в файлах конфигурации и хранения данных. К особенностям JSON можно отнести то, что он:
...
+ представляет данные в виде набора пар ключ-значение. Каждый ключ - это строка, а значения могут быть строками, числами, булевыми значения, объектами, массивами или null. Эти наборы пар заключены в фигурные скобки {}
...
3. **Булевые значения**: В JSON есть два булевых значения: true (истина) и false (ложь).
...
1. Сложность и тяжеловесность: SOAP использует XML для форматирования сообщений, что делает его более тяжеловесным по сравнению с более современными и легковесными форматами данных, такими как JSON. Это требует больше ресурсов для обработки и передачи данных.
...
5. Проблемы с производительностью: Из-за большего объема данных и сложности обработки XML SOAP-сообщений могут быть менее эффективными с точки зрения производительности по сравнению с RESTful API.
6. Сложность в настройке безопасности: Хотя SOAP поддерживает WS-Security для обеспечения безопасности сообщений, его конфигурация и настройка могут потребовать дополнительных усилий и специализированных знаний.
7. Менее подходит для ресурсоемких операций: Использование SOAP для крупных и ресурсоемких операций может повлиять на производительность из-за высокой нагрузки на сеть и процессор.
### gRPC
gRPC (gRPC Remote Procedure Calls) — современный, высокопроизводительный RPC (Remote Procedure Call) фреймворк, разработанный Google в 2015 году. Он использует ```HTTP/2``` для транспорта и ```Protocol Buffers``` (protobuf) для сериализации данных.
#### Преимущества gRPC
1. Высокая производительность: 
   ...
   2. Protocol Buffers:
      1. обеспечивают компактное представление данных в бинарном формате, что уменьшает размер передаваемых данных и ускоряет их передачу по сети
      2. благодаря простоте и эффективности бинарного формата обеспечивают более компактное и быстрое кодирование/декодирование данных по сравнению с JSON или XML.
...
5. Интероперабельность: Благодаря использованию Protocol Buffers и стандартов HTTP/2, gRPC обеспечивает высокую степень совместимости между различными системами и сервисами.
#### Недостатки gRPC
...
2. Ограниченная поддержка браузеров: Большинство современных веб-браузеров поддерживают HTTP/2, однако они не поддерживают напрямую протокол gRPC, используемый для RPC в бэкенд-серверных приложениях (т.к. протокол gRPC обычно использует бинарный формат для передачи данных и требует специализированной обработки, которая не поддерживается непосредственно браузерами). Для того чтобы клиентские веб-приложения могли использовать gRPC для взаимодействия с бэкендом через браузеры, был разработан протокол ```gRPC-Web```, являющийся адаптацией gRPC для работы через стандартный HTTP/1.x (вместо HTTP/2), который поддерживается браузерами. Это позволяет клиентским веб-приложениям взаимодействовать с серверными приложениями, используя gRPC, несмотря на ограничения браузеров в поддержке HTTP/2 и бинарного gRPC-протокола. Однако для использования gRPC-Web требуется дополнительный прокси-сервер или промежуточный слой на стороне сервера, который конвертирует запросы и ответы между gRPC и gRPC-Web. Этот прокси-сервер или промежуточный слой позволяет преобразовывать запросы HTTP/1.x от клиентских браузеров в gRPC-запросы на сервере и наоборот.
...
+ клиент может отправлять запросы различными способами, в зависимости от используемых инструментов и среды разработки, например, с помощью Postman;
...
```
  3. Введите URL вашего GraphQL сервера, например, http://localhost:4000/graphql.
  4. Нажмите "Send" для выполнения запроса.
...
2. Гибкость запросов: Клиенты могут комбинировать несколько запросов в одном вызове, что позволяет уменьшить количество сетевых запросов.
...
5. Реализация вложенных запросов: GraphQL позволяет клиентам делать вложенные запросы, получая связанные данные в одном запросе. Это особенно полезно для сложных данных с вложенными отношениями.
6. Инструменты для разработки: GraphQL имеет отличные инструменты для разработки и отладки, такие как GraphiQL и Apollo Client, которые помогают разработчикам быстро и эффективно работать с API.
#### Недостатки GraphQL
...
5. Избыточные данные: Хотя клиенты могут запрашивать только необходимые данные, неправильное использование GraphQL может привести к запросу избыточных данных, что может негативно сказаться на производительности.
...
Endpoint (конечная точка) в API — это конкретный URL-адрес, по которому можно определенное действие в рамках API(получение данных, создание ресурса, обновление или удаление).
...
##### Инструменты и спецификации динамической документации:
  + ```OpenAPI``` - спецификация для описания RESTful API. Определяет стандартный формат для создания машинно-читабельной документации API, включающей в себя информацию о всех эндпоинтах, методах HTTP, параметрах, типах данных и так далее. Основные аспекты OpenAPI: 
    + Спецификация: OpenAPI Specification (OAS) предоставляет язык для описания интерфейсов API в формате JSON или YAML.
    + Версии: Текущая версия OpenAPI — 3.x, которая значительно улучшена по сравнению с версией 2.0.
    + Цель: унифицировать описание API для понимания, генерации клиентских библиотек, документации и тестовых инструментов.
  + ```Swagger``` - набор инструментов и экосистема для работы с API, созданная для поддержки и расширения возможностей, описанных в спецификации OpenAPI. Включает в себя следующие компоненты:
    + Swagger Editor: Веб-редактор для создания и редактирования спецификаций OpenAPI.
    + Swagger UI: Генератор пользовательского интерфейса для взаимодействия с API, описанным в спецификации OpenAPI. Этот инструмент позволяет пользователям легко тестировать API через веб-интерфейс.
    + Swagger Codegen: Инструмент для генерации клиентских библиотек, серверных стубов и документации из спецификации OpenAPI.
    + Swagger Inspector: Инструмент для тестирования API и генерации спецификаций OpenAPI из существующих API.
### Примеры
...
### Токены
Токены в контексте программирования и сетевых технологий — это уникальные строки, используемые для аутентификации, авторизации и управления сессиями. Токены позволяют подтверждать личность пользователя или приложения и управлять доступом к ресурсам. Существует несколько типов токенов, которые применяются в разных сценариях.
**API-токен** - это часть данных, которая используется для аутентификации и авторизации субъекта (пользователя, системы или приложения) при взаимодействии с API. **API-токены** обычно используются для обеспечения безопасности и контроля доступа. Они действуют как форма учетных данных, позволяя владельцу токена делать запросы к API и получать доступ к определенным данным или сервисам. **Ключевые моменты касающиеся API-токенов**:
...
4. *Типы API-токенов*: Существуют различные типы API-токенов, в том числе:
   1. *API-ключ (API Keys)*: Простая строка или код, который часто включается в заголовок запроса. Ключи API обычно используются для идентификации и проверки подлинности клиента, выполняющего запрос. Передаются в заголовке запроса или в URL-параметрах.
   2. *Токен носителя (Bearer Tokens)*: Тип аутентификации на основе токенов, при котором токен предъявляется в качестве доказательства личности. Передаются в заголовке ```Authorization``` с префиксом ```Bearer```
   3. *JWT (JSON Web Tokens)*: Компактные, URL-безопасные токены, состоящие из заголовка, полезной нагрузки и подписи. Передаются в заголовке Authorization с префиксом Bearer или в теле запроса.
   4. *OAuth-токены*: Токены, используемые в протоколе OAuth для предоставления ограниченного доступа к ресурсам пользователя. ```access_token``` используется для доступа к API, ```refresh_token``` используется для получения нового ```access_token```.
#### API-ключ
