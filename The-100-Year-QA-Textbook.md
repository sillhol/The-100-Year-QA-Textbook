...
+ ```date```	-	используется для:
  ...
  + Установки системной даты и времени (требуются права суперпользователя): `sudo date -s "2024-08-30 18:11:01"`
...
+ ```pwd```	-	используется для вывода текущей рабочей директории
  ...
  + `pwd -P` : показывает физический путь, без учета символических ссылок
  + `pwd -L` : показывает логический путь, учитывая символические ссылки (по умолчанию).
...
+ ```grep```	-	используется для поиска строк в текстовых файлах или стандартном вводе, которые соответствуют заданному шаблону. Поиск осуществляется либо исходя из Basic Regular Expressions (```grep "pattern" filename.txt```) либо Extended Regular Expressions (```grep -E "regex_pattern" filename.txt```):
...
+ ```ln -sf three/one.txt two/one.txt```	-	создает символическую ссылку на файл one.txt в директории three, и делает эту ссылку вместо уже существующего файла one.txt в директории two, если он существует. Вот что означают использованные опции
  ...
  + Пример поиска всех файлов в системе, которые имеют тот же inode, что и указанный файл (т.е. являются **ЖЁСТКИМИ ССЫЛКАМИ**) 
    + `find / -inum $(stat -c %i /path/to/file)` Здесь:
      + `stat -c %i /path/to/file`: Эта часть команды выводит inode указанного файла. Параметр `-c %i` указывает stat вывести только inode файла.
      + `$(...)`: Это командная подстановка, которая позволяет использовать результат выполнения команды внутри другой команды. В данном случае, результатом будет inode файла.
