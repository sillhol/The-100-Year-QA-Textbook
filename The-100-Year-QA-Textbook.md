...
### Процессы и демоны
#### Процессы
**Процесс** операционной системы — это экземпляр исполняемой программы, выполняемый системой в настоящее время. Если программа заканчивает свою работу, то процесс также прекращает своё существование.
...
+ `CMD` (Имя команды, запустившая процесс). 
...
+ `-s, --sid <session>`: отображает процессы для заданной сессии.
...
+ `-p <PID>` : отображает процессы, запущенные пользователем с заданным `PID`
+ `--ppid <PPID>` : отображает процессы, запущенные пользователем с заданным идентификатором родительского процесса `PPID`
+ `-C <command>`: отображает процессы, запущенные указанной командой `ps -C sshd`
+ `-t <tty>`: отображает процессы, связанные с указанным терминалом `ps -t pts/0`
