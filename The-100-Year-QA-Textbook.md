...
1. Определяется через файл `.service`, который `systemd` использует для управления процессом или приложением, содержащим следующие разделы:
   ...
   2. `[Service]` используется для определения параметров и команд, необходимых для запуска, остановки и перезапуска сервисов.
      1. Что делать при запуске (`ExecStart`): выполнить программу `/путь/к/исполняемому_файлу`.
      2. Как завершить процесс (`ExecStop`): отправить сигнал завершения.
      3. Что делать при сбое (`Restart`): перезапускать процесс.
   3. `[Install]` описывает, как и когда юнит должен быть установлен активирован в системе. Он содержит информацию о том, к каким целям (targets) юнит должен быть привязан и в какие моменты его следует запускать.
...
4. Пример структуры файла `.service` (Unit-файла сервиса)
    ...
    Requires=network.target my-socket.socket 
    (Определяет другие юниты, которые должны быть активны для запуска   данного юнита. Если один из них не удается запустить или он завершился неудачно, текущий юнит также завершится)
    Conflicts=another-service.service (Обратное к Requires. Определяет юниты, которые не могут быть активными одновременно с текущим юнитом. Если один из них активируется, другой будет остановлен)
    Wants=postgresql.service (Юниты, указанные в Wants, будут пытаться запускаться вместе с текущим юнитом, но если один из них не удается запустить, это не приведет к ошибке текущего юнита)
    After=network.target (Определяет порядок запуска. Текущий юнит начнет запускаться после юнитов из поля After. Однако это не означает, что указанные юниты должны быть запущены — только что текущий юнит не будет запускаться раньше них)
    Before=nginx.service (Обратное к After)
    ConditionPathExists=/etc/my-sample-service/config (Указывает условие, проверяющее существование файла или каталога. Если указанный путь не существует, юнит не будет запускаться)

    [Service]
    Type=simple (или другие типы: simple (по умолчанию): Process, указанный в ExecStart, запускается и остаётся активным; forking: Процесс запускает другую программу и завершается; oneshot: Для служб, которые выполняются и завершаются сразу; dbus: Процесс должен зарегистрироваться с D-Bus; notify: Процесс уведомляет systemd о своём состоянии; idle: Выполняется, когда загрузка системы завершена)
    ExecStart=/путь/к/исполняемому_файлу (основная команда для запуска сервиса. Это обязательное поле)
    ...
    Restart=always (указывает, как service manager должен поступать в случае аварийного завершения службы: no (Служба не будет перезапущена, независимо от причины завершения. Это поведение по умолчанию), on-success (Служба будет перезапущена только в случае её успешного завершения, то есть если она завершилась с кодом 0 или сигнала TERM), on-failure (Служба будет перезапущена в случае её неуспешного завершения, то есть если она завершилась с любым кодом, отличным от 0, или если она была завершена сигналом, отличным от TERM), on-abnormal (Служба будет перезапущена, если она была завершена из-за сигнала, который не был вызван системой управления службами. Например, если служба была завершена из-за сигнала SEGV или ABRT), on-watchdog, on-abort (Служба будет перезапущена, если она была завершена из-за получения сигнала ABRT (abort)), или always (Служба будет перезапущена в любом случае, независимо от причины её завершения.))
    ExecStartPre=/путь/к/исполняемому_файлу_предварительно (команда, выполняемая перед запуском основного процесса. Полезно для выполнения предварительных задач, таких как проверка условий или подготовка среды)
    ExecStartPost=/путь/к/исполняемому_файлу_после (команда, выполняемая после успешного запуска основного процесса)
    RestartSec=5 (определяет задержку (в секундах) перед перезапуском службы после ее неуспешного завершения или остановки. Это поле используется совместно с параметром Restart и помогает предотвратить слишком частые перезапуски службы в случае, если она часто выходит из строя)
    ...
    [Install]
    WantedBy=multi-user.target (WantedBy связывает ваш сервис с определенной целью, указывая systemd запускать ваш сервис вместе с этой целью. Но если ваш не сможет запуститься это не будет критически важным для цели)
    RequiredBy=multi-user.target (RequiredBy связывает ваш сервис с определенной целью, указывая systemd запускать ваш сервис вместе с этой целью. Но если ваш не сможет запуститься это будет критически важным для цели и цель не будет считаться полностью активированной без вашего юнита)
    Also=another.service (используется для указания дополнительных юнитов, которые должны быть установлены вместе с текущим юнитом)
...
+ `indirect`: служба является ссылкой на другую `unit`
+ `generated`: служба была создана динамически во время запуска системы.
