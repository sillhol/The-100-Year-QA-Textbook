...
### Встроенная команда оболочки (built-in commands)
...
### Псевдонимы (Алиас) оболочки
**Алиас оболочки** - Это определенное пользователем имя существующей команды (и, возможно, ее опций). Если "b" является псевдонимом для "a --option1 --option2", то когда оболочке входа в систему предлагается выполнить "b --some-options arguments", на самом деле будет выполнено "a --option1 --option2 --some-options arguments". 

Часто система Linux настроена так, что некоторые псевдонимы для всех пользователей доступны сразу же, например, `ll` (`ls -l`), `la` (`ls -a`).
...
+ `TTY` (Управляющий терминал). Указывается для процессов, взаимодействующих с терминалом и (или) пользователями в диалоговом режиме
  ...
  + Процессы, не имеющие `TTY`
    + Серверные или фоновые программы (демоны) не используют управляющий терминал, так как они не предполагают прямого взаимодействия с пользователем. Если демон правильно настроен, он:
      + отвяжется от терминала, с применением одного из способов (или их комбинации):
        + `nohup`: Простое средство, чтобы процесс продолжал работать после закрытия терминала
        + `setsid()`: Инструмент для создания нового сеанса и отвязывания от терминала
        + `daemon()`: Полноценный способ демонизации процессов, который включает `setsid()` и дополнительные действия, такие как перенаправление потоков
      + Перенаправит свои стандартные потоки (`stdin`, `stdout`, `stderr`) в `/dev/null` или файлы
    + Программы, запускаемые с перенаправлением всех стандартных потоков (`stdin`, `stdout`, `stderr`) в `/dev/null` или файлы. 
      + Но если хотя бы один поток не перенаправлен и остаётся связанным с `TTY`, то и весь процесс также всё ещё привязан к `TTY`
...
+ `-o` задает список полей для вывода определённых сведений о процессах (`ps -o pid,ppid,cmd,%cpu,%mem`):
  ...
  + `stat`: состояние процесса. Варианты вывода:
    ...
    + `D`: Uninterruptible sleep (процесс ожидает завершения критической операции ввода/вывода, такой как запись на диск, и не может быть прерван по сигналу)
...
#### `killall` 
`killall` - отправляет один и тот же сигнал нескольким процессам (в том числе с одинаковым названием), более подробно синтаксис функции 
```console
...
+ `-u, --user <userName>` - Завершить процессы, принадлежащие указанному пользователю. Имена команд необязательны.
...
#### pkill
`pkill` - отправляет один и тот же сигнал нескольким процессам, названия которых отвечают некоторому условию. Похожа на команду `pgrep`. , более подробно синтаксис функции: `pkill [-ЧТО] [-fvx] [-n|-o] [-P ppid,...] [-g pgrp,...][-s sid,...] [-u euid,...] [-U uid,...] [-G gid,...][-t term,...] [pattern]`
+ `-f` - Искать в полной командной строке (по умолчанию шаблон сопоставляется только с именем процесса)
