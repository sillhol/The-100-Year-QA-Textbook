...
#### Диаграмма переходов между состояниями TCP в зависимости от полученных и отправленных пакетов:
1. LISTEN:
...
**Кортеж**, который формируют значения колонок ```Proto```, ```LocalAddress``` и ```Foreign Address``` образуют, можно увидеть в выводе результата командой ```netstat```:
```console
$ netstat -pnt
...
+ Wireshark схожа с ```tcpdump``` и наличием фильтра пакетов. Логика составления выражений во многом та же, но синтаксис несколько отличается:
  ...
  +   [Как использовать возможности фильтров отображения Wireshark по максимуму](https://xakep.ru/2013/11/05/wireshark-filtres/)
