...
### traceroute
**traceroute** — это сетевая утилита, показывающая маршрут, по которому пакеты проходят от исходного хоста до целевого узлом в IP-сети (показывает каждый промежуточный узел (хоп) между вами и целевым узлом, а также время, затраченное на каждый хоп). Это позволяет выявить узкие места, задержки или точки отказа в сети.
